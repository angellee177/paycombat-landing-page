...
export function HowItWorksSection({ headline, description, steps }: HowItWorksProps) {
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col lg:flex-row gap-16 items-start">
          <div className="lg:w-1/3 space-y-6">
            {headline ? <h2 className="text-4xl font-extrabold tracking-tight">{headline}</h2> : null}
                      ...
                      >
                        <span
                          className={`w-8 h-8 rounded-full flex items-center justify-center text-xs ${
                            isActive ? 'bg-primary text-on-primary' : 'bg-surface-dim text-on-surface'
                          }`}
                        >
                          {index + 1}
                        </span>
          ...
          <div className="lg:w-2/3 grid grid-cols-1 gap-8">
            {activeStep ? (
              <article
                key={activeStep.id ?? activeStep.label ?? activeStep.title ?? `detail-${activeStepIndex}`}
              ...
              >
                ...
                {activeStep.title ? <h4 className="text-xl font-bold mb-2">{activeStep.title}</h4> : null}
                {activeStep.description ? (
                  <p className="text-on-surface-variant text-sm font-medium">{activeStep.description}</p>
                ) : null}
              </article>
            ) : null}
          </div>
